 ...
 * LLM as a judge (JLLM) for custom evaluation
...
Thus the agent supports multiple steps:
1. schema and sample data retrieval from the indexes 
2. text to sql generation 
3. sql retrieval (execution) from the table(s)
4. response generation from the structured data retrieved with sql

Ultimately, the text-to-SQL agent implements end-to-end RAG pipeline on arbitrary structured data sources.
This complexity includes various configurations and customizations that may affect agent's correctness, efficiency, and performance.

## Risks

We foresee certain risks and shortcomings such as:
...
To perform comprehensive evaluation of the agent we employed specialized framework for RAG solutions - RAGChecker (see `ragchecker_eval.py`).
With this framework we are able to measure main components of RAG pipeline:
...
For this exercise we focused on the overall metrics: precision, recall, and F1 (geometric average of precision and recall)
